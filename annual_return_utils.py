...
# Purpose:     Measure yield of a portfolio over the time.  Mesure the Trailing
...
#-------------------------------------------------------------------------------
# Description: Define a table with the portfolio on dates when a transaction occurs
#
...
class CCalcReturnsStocks:
    ...
    def getAnnualReturn(self):
        self.portfolioSnapshot = getPortfolioSnapshot(self.dataTransactions, self.startDate)
        return self.portfolioSnapshot
        #return getAnnualReturn(self.portfolioSnapshot, self.endDate)
...
def _main():
    ...
    dt = CCalcReturnsStocks(dataTransactions, '01/01/2014', '01/01/2016')
